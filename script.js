...
var loss = false;
var Elapsed_time=0;
var Elapsed_time_text = "your Elapsed time is:"
var Elapsed_time_text_end = "s";
var btn_tryagain = "Try Again"
var btn_nex = "Next Level";
var game_win = "you Win.good job"
...
$(function () {
	...
	$("#tryAgain").click(function () {
		...
		Elapsed_time += timer;
		Elapsed_time-=1;
		if ($("#tryAgain").html()=="Try Again" || $("#tryAgain").html()=="تلاش دوباره") {
			alert(`${Elapsed_time_text}${Elapsed_time}${Elapsed_time_text_end}`);
			Elapsed_time=0;
		}
		timer=0;
		$(".cell").remove();
	...
	$("#change_language").change(function () {
		...
		if (language=="persian") {
			...
			$("#language p").html("زبان");
			$("#language_flag").css({
				"background-image":"url(flag/iran.jpg)"
			})
			$("#timeLeft").html("زمان شما برای حل جدول:←");
			$("#timeLeft").css({
				"right": "15%"
			})
			$("#Play").html("شروع(کلیک)←");
			$("#level").html(":مرحله");
			if($("#tryAgain").html()=="Next Level"){
				btn_nex = "مرحله بعد";
				game_win = "شما برنده شدید.کار شما خوب بود";
				$("#tryAgain").html(btn_nex);
				$("#result").html(game_win);
			}
			else if($("#tryAgain").html() == "Try Again"){
				btn_tryagain = "تلاش دوباره";
				game_over = "شما باختید";
				$("#tryAgain").html(btn_tryagain);
				$("#result").html(game_over);
			}
			Elapsed_time_text="زمان سپری شده شما:"
			Elapsed_time_text_end = "ثانیه";
		}
		else if(language=="English"){
			$("html").css({
				"direction":"ltr"
			})
			$("#language p").html("English");
			$("#language_flag").css({
				"background-image":"url(flag/england.jpg)"
			})
			$("#timeLeft").html("←Your time to solve this table");
			$("#timeLeft").css({
				"right": "7%"
			})
			$("#Play").html("←Play(click)");
			$("#level").html("level:");
			if($("#tryAgain").html()=="مرحله بعد"){
				btn_nex = "Next Level";
				game_win = "you Win.good job";
				$("#tryAgain").html(btn_nex);
				$("#result").html(game_win);
			}
			else if($("#tryAgain").html()=="Try Again"){
			...
			Elapsed_time_text = "your Elapsed time is:"
			Elapsed_time_text_end = "s";
		}
	})
});
function reloadSell() {
